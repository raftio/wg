...
impl MySqlBackend {
    ...
    pub async fn new(database_url: &str, namespace: &str) -> Result<Self> {
        let pool = MySqlPoolOptions::new()
            ...
            .await
            .map_err(|e| WgError::Config(format!("Failed to connect to MySQL: {}", e)))?;
    ...
    async fn pop_job(&self, timeout: Duration) -> Result<Option<String>> {
        ...
        loop {
            ...
            let mut tx = self.pool.begin().await.map_err(|e| {
                WgError::JobProcessing(format!("Failed to begin transaction: {}", e))
            })?;
            ...
            .await
            .map_err(|e| WgError::JobProcessing(format!("Failed to select job: {}", e)))?;
            ...
            if let Some((id, job_json)) = result {
                ...
                sqlx::query(&format!("DELETE FROM {} WHERE id = ?", self.jobs_table()))
                    ...
                    .execute(&mut *tx)
                    .await
                    .map_err(|e| WgError::JobProcessing(format!("Failed to delete job: {}", e)))?;

                tx.commit()
                    .await
                    .map_err(|e| WgError::JobProcessing(format!("Failed to commit: {}", e)))?;
            ...
            tx.rollback()
                .await
                .map_err(|e| WgError::JobProcessing(format!("Failed to rollback: {}", e)))?;
